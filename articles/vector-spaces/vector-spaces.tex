...
A function $f$ from $\R^n$ to $\R$ is said to be continuous if it is continuous with respect to their usual topologies.
...
\where
	\forall n: \nat @ \\
	\t1	\CzeroN(n) = \CzeroTT(\tsRtup(n), \Rtau)
\end{axdef}
...
\begin{axdef}
	\CzeroPRn: \DeltaRinf \fun \power(\Rinf \pfun \R)
\where
	\forall U: \DeltaRinf @ \\
	\t1	\LET n == \dimRinf U @ \\
	\t2		\CzeroPRn(U) = \CzeroTT(\tsRtup(n) \inducedTopSp U, \Rtau)
\end{axdef}
...
\where
	\forall x: \Rinf @ \\
	\t1	\LET n == \# x @ \\
	\t2		\CzeroRn(x) = \{~ f: \Rtup(n) \pfun \R | \exists U: \neighRinf(x) | U \subseteq \dom f @ U \dres f \in \CzeroPRn(U) ~\}
\end{axdef}
...
\where
	\forall n, m: \nat @ \\
	\t1	\CzeroNN(n,m) = \CzeroTT(\tsRtup(n), \tsRtup(m))
\end{axdef}
...
\begin{zed}
	\forall n, m: \nat@ \\
	\t1	\linRtup(n, m) \subseteq \CzeroNN(n,m)
\end{zed}
...
Let $\CzeroPRnN(U,m)$ denote the set of continuous mappings from the topology induced by $\tsRtup(n)$ on $U$ to $\tsRtup(m)$.

\begin{axdef}
	\CzeroPRnN: \DeltaRinf \cross \nat \fun \power (\Rinf \pfun \Rinf)
\where
	\forall n, m: \nat @ \\
	\t1	\forall U: \DeltaRinf | \dimRinf(U) = n @ \\
	\t2		\CzeroPRnN(U, m) = \CzeroTT(\tsRtup(n) \inducedTopSp U, \tsRtup(m))
\end{axdef}
...
\begin{zed}
	\forall n, m: \nat @ \\
	\t1	\CzeroPRnN(\Rtup(n),m) = \CzeroNN(n,m)
\end{zed}
...
Let $\CzeroRnN(x,m)$ denote the set of all partial functions $f$ from $\Rtup(n)$ to $\Rtup(m)$
