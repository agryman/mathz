...
In general, an \textit{algebraic structure} consists of one or more sets of elements 
equipped with one or more objects, such as operations, defined on them.
A group is an algebraic structure consisting of one set of elements
equipped with one binary operation, typically referred to
...
\also
	int\_mul\_square \in magma\_Map[\num, \num]
\end{zed}
...
\begin{zed}
	\forall x, y: \num @ int\_square(x*y) = int\_square(x) * int\_square(y)
\end{zed}
...
\begin{figure}[h]
    \begin{tikzcd}
        A \arrow[r, "f"] \arrow[rd, "g"'] & A' \arrow[d, "f'"] \\
        & A''
    \end{tikzcd}
...
\begin{figure}[h]
    \begin{tikzcd}
        \strucA \arrow[r, "F"] \arrow[rd, "G"'] & \strucA' \arrow[d, "F'"] \\
        & \strucA''
    \end{tikzcd}
    \caption{Commutative diagram for the composition $G$ of $F'$ after $F$.}
...
\begin{example}[Subimage of $int\_add\_square$]
