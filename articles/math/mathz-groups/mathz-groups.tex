...
\begin{zed}
	int\_add\_nat\_id \in MAGMA\_MAP[\num, \num]
\end{zed}
...
The previously defined structure $int\_add\_nat\_id$ is not a magma map because the
domain of its underlying function, namely $\id \nat$, is only the proper subset  $nat$ of $num$.
...
\begin{zed}
	int\_square == (\lambda x: \num @ x * x)
\end{zed}

Define $int\_add\_square$ to be the following candidate magma map structure.

\begin{zed}
	int\_add\_square == (int\_add, int\_add) \mapsto int\_square
\end{zed}

This structure is in fact a magma map because its underlying map
is defined on all elements of the source carrier.

\begin{zed}
	int\_add\_square \in magma\_Map[\num, \num]
\end{zed}
...
\begin{counterexample}
$int\_add\_square$ is not a magma homomorphism because, in general, the sum of squares
...
\begin{zed}
	\forall c: \num @ \\
	\t1	int\_add\_mul\_const(c) \in magma\_hom(int\_add, int\_add)
\end{zed}
...
\begin{zed}
	\forall a, b: \num @ \\
	\t1	int\_add\_mul\_const(a) \compMagma int\_add\_mul\_const(b) = int\_add\_mul\_const(a * b)
\end{zed}
...
\begin{example}[Natural Number Arithmetic]
...
\begin{zed}
	nat\_int\_add \in magma\_Inclusion[\num]
\also
	nat\_int\_mul \in magma\_Inclusion[\num]
\end{zed}
...
is the magma subset $\strucS'$ of $A'$ that consists of
...
\begin{example}[Image of $int\_add\_square$]

\begin{zed}
	magma\_Image(int\_add\_square) = (int\_add, \ran int\_square)
\end{zed}
