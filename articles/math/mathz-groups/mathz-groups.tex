...
Define $pbin\_op[\genT]$ to be the set of all partial binary operations in $\genT$.
...
Define $bin\_op[\genT]$ to be the set of all binary operations on $\genT$.
...
Define $magma\_map(\strucA, \strucA')$ to be the subset of all magma maps from $\strucA$ to $\strucA'$.
...
\begin{remark}
$magma\_map(\strucA, \strucA')$ is a subset of $magma\_Map[\genT, \genU]$.

\begin{zed}
	\forall Magma[\setT]; Magma'[\setU] @ \\
