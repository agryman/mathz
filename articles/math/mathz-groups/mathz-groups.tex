...
\begin{counterexample}[Integer Division and Modulus]
...
\begin{example}[Integer Arithmetic]
...
\begin{zed}
	(\LET A == \num; (\_ \opG \_) == (\_ + \_); \strucA == int\_add @ Magma[\num])
\also
	(\LET A == \num; (\_ \opG \_) == (\_ - \_); \strucA == int\_sub @ Magma[\num])
\also
	(\LET A == \num; (\_ \opG \_) == (\_ * \_); \strucA == int\_mul @ Magma[\num])
\end{zed}
...
\begin{example}[Integer Arthmetic]
...
\begin{example}[Identity Maps]
The identity map on the set of integers extends to a magma map structure from any magma on the integers
to itself. Define these structures for integer addition, subtraction, and multiplication.

\begin{zed}
	int\_add\_id == (int\_add, int\_add) \mapsto \id \num
\also
	int\_sub\_id == (int\_sub, int\_sub) \mapsto \id \num
\also
	int\_mul\_id == (int\_mul, int\_mul) \mapsto \id \num
\end{zed}

These structures all have type $MAGMA\_MAP[\num, \num]$.

\begin{zed}
	int\_add\_id \in MAGMA\_MAP[\num, \num]
\also
	int\_sub\_id \in MAGMA\_MAP[\num, \num]
\also
	int\_mul\_id \in MAGMA\_MAP[\num, \num]
\end{zed}

\end{example}

TODO: This is too repetitive. It would be better to move the section on identity maps here
and use it to construct identity map structures. Then prove that they are magma maps and homomorphisms.
Alternatively, move the $MulConst$ and $ExpConst$ examples here and defer the discussion of 
identity maps. For example, define standalone integer partial and total functions, then use them to construct
$MAGMA\_MAP$ objects. One should fail to be a total function, hence not a map.
One should be a map but fail to be a homomorphism. One should be a homomorphism.

We further require the underlying map $f$ of a magma map structure $F$
from $\strucA$ to $\strucA'$ to be a function from $A$ to $A'$.
...
\begin{remark}
The identity map on the carrier of a magma is a function from the carrier to itself.
...
\begin{example}
The previously defined identity map structures are magma maps.
...
\begin{example}
The previously defined identity map structures are magma maps from each magma to itself.

\begin{zed}
	int\_add\_id \in magma\_map(int\_add, int\_add)
\also
	int\_sub\_id \in magma\_map(int\_sub, int\_sub)
\also
	int\_mul\_id \in magma\_map(int\_mul, int\_mul)
\end{zed}
...
\begin{example}
The previously defined identity map structures are magma homomorphisms.

\begin{zed}
	int\_add\_id \in magma\_Hom[\num, \num]
\also
	int\_sub\_id \in magma\_Hom[\num, \num]
\also
	int\_mul\_id \in magma\_Hom[\num, \num]
\end{zed}
...
\begin{example}
The previously defined identity map structures are magma homomorphisms from each magma to itself.

\begin{zed}
	int\_add\_id \in magma\_hom(int\_add, int\_add)
\also
	int\_sub\_id \in magma\_hom(int\_sub, int\_sub)
\also
	int\_mul\_id \in magma\_hom(int\_mul, int\_mul)
\end{zed}
...
The \textit{image} of a magma map $f$ from $A$ to $A'$
is the subset $S'$ of $A'$ that consists of
the image under the map of the elements in $A$.
...
Define $magma\_Image$ to be the function that associates any magma map $F$ with its image $\strucS'$.

\begin{zed}
	magma\_Image[\genT, \genU] == \{~ Magma\_Image[\genT, \genU] @ F \mapsto \strucS' ~\}
\end{zed}

Define $magma\_image(\strucA, \strucA')$ to be the function that associates any magma map $F$
from $\strucA$ to $\strucA'$ with its image $\strucS'$ in $\strucA'$.

\begin{zed}
	magma\_image[\genT, \genU] == \\
	\t1	(\lambda \strucA: magma[\genT]; \strucA': magma[\genU] @ \\
	\t2		magma\_map(\strucA, \strucA') \dres magma\_Image[\genT, \genU])
\end{zed} 
...
\begin{zed}
	...
	\t2		magma\_image(\strucA, \strucA')~F \in magma\_submagma(\strucA')
\end{zed}
